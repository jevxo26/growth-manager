...
export async function GET(request) {
  ...
  if (fetchStats) {
    ...
    const [totalCount, approvedCount, productCount] = await Promise.all([
      WpLead.countDocuments(query),
      WpLead.countDocuments({ ...query, isApproved: true }),
      WpLead.countDocuments({ ...query, willTakeProduct: true }),
    ]);
    stats = {
      total: totalCount,
  ...
  if (fetchAll) {
    leads = await WpLead.find(query).sort({ sentAt: -1 }).lean();
    total = leads.length;
  } else {
    const skip = (page - 1) * limit;
    total = await WpLead.countDocuments(query);
    leads = await WpLead.find(query)
      .sort({ sentAt: -1 })
      .skip(skip)
      .limit(limit)
      .lean();
