...
export default function WpLeadsClient() {
  ...
  async function fetchLeads() {
    ...
    try {
      ...
      const res = await fetch(`/api/wp-leads?${params.toString()}`);
      if (res.ok) {
  ...
  async function handleToggle(id, field, currentValue) {
    ...
    try {
      const res = await fetch(`/api/wp-leads/${id}`, {
        method: "PATCH",
  ...
  async function exportToExcel() {
    try {
      ...
      XLSX.utils.book_append_sheet(workbook, worksheet, "WP Leads");
      XLSX.writeFile(workbook, "WP_Leads_Export.xlsx");
    } catch (err) {
  ...
  return (
    <div className="space-y-6">
      <div className="flex flex-col gap-4 sm:flex-row sm:items-center sm:justify-between">
        <div>
          <h1 className="text-2xl font-bold tracking-tight text-slate-900">WP Leads</h1>
          <p className="text-sm text-slate-500">
            Track and manage people who received your WhatsApp promotions.
          </p>
        </div>
